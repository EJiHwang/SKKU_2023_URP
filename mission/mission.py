...
def get_fitline(img, f_lines):
    ...
    x1, y1 = int(((img.shape[0] - 1) - y) / (vy + 0.0001) * vx + x), img.shape[0] - 1
    x2, y2 = int(((img.shape[0] / 2 + 100) - y) / (vy+0.0001) * vx + x), int(img.shape[0] / 2 + 100)
    result = [x1, y1, x2, y2]
...
def ROI_cutter(img, point1, point2):  # 230717 리사이징 없이 ROI이미지를 만듭니다 
...
def ROI_cutter2(img, point1, point2):  # (640*480) # 230717 리사이징을 하면서 ROI 이미지를 만듭니다
...
while True:
    ...
    if (traffic_counter % 4 == 0):  # 원래 %4 였음
        ret, frame2 = cap2.read()
        # cv2.imshow('frame2', frame2)
        imgROI = ROI_cutter(frame2, (0, 360), (300, 480))
        imgROI = ROI_cutter2(frame2, (0, 360), (300, 480)) #원래는 300, 480
        result = object_detection(img=imgROI, sample=16, print_enable=True)
        # imgBlur = gaussian_blur(imgROI, 3)
        cv2.imshow("ROI img", imgROI)
        # result = object_detection(imgROI, sample=16, print_enable=True)
        if result == "RED":
            print("red")
    ...
    if ((direction != d1) & (direction != None) & (direction != 0)):
        ...
        if direction == 1:
            ...
            direc = "i"
        elif direction == 10: # red signal
            direc = "j"
        elif direction == 11: # green signal
            direc = "k"

        #data = comm.read().decode()
        #print(data)
